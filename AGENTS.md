...
- Use `shell: powershell` for Windows workflow steps. The startup-launched runner inherits only the persistent Windows PATH and must not depend on Codex's private `pwsh.exe` runtime path.
