...
- Alert discovery should anchor on visible ticker elements and the nearest alert row, deduplicate identical DOM nodes, and log row diagnostics when selectors match. Current TradingView alert rows use a hashed `itemBody-*` class, so keep `[class*="itemBody"]` as a row fallback and verify the selected `Alerts` tab rather than assuming a ticker selector is complete. Alert deletion succeeds only when the target ticker count decreases; same-name alerts are deleted one at a time and each `beforeCount`/`afterCount` is logged.
