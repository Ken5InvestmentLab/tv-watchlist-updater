...
- Keep the per-user Startup launcher and the 09:05 `TVWatchlistLocalRunner` readiness task together: the launcher recovers after a missed logged-out schedule, while the daily task restarts a stopped runner before the 09:17 dispatch when the user session is active.
- Keep Discord payload construction in `scripts/notify-discord.js` and invoke it through the workflow's ASCII-only `cmd` step. Windows PowerShell 5.1 misreads the runner's UTF-8-without-BOM temporary scripts when inline YAML contains Japanese text or emoji, and the startup-launched runner must not depend on Codex's private `pwsh.exe` path.
