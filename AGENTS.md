...
- In CDP mode, create an updater-owned tab and never close or navigate the user-visible Edge window. Close only the updater-owned tab and disconnect Playwright after `DONE` so the runner step can exit. The ignored `.local-runner/` directory and the Edge profile are local operational state, not repository artifacts.
